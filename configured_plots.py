...
#        plots.extend(gps_plots)
    ...
    # Position plot
    data_plot = DataPlot2D(data, plot_config, '飞机位置',
                           x_axis_label='[m]', y_axis_label='[m]', plot_height='large')
    data_plot.add_graph('y', 'x', colors2[0], '位置',
                        check_if_all_zero=True)
    if not data_plot.had_error: # vehicle_local_position is required
        data_plot.change_dataset('飞机位置期望值')
        data_plot.add_graph('y', 'x', colors2[1], '期望位置')
    ...
    # Altitude estimate
    data_plot = DataPlot(data, plot_config, '飞机GPS坐标',
                         y_axis_label='[m]', title='飞行高度',
                         changed_params=changed_params, x_range=x_range)
    data_plot.add_graph([lambda data: ('alt', data['alt']*0.001)],
                        colors8[0:1], ['GPS高度'])
    data_plot.change_dataset(baro_alt_meter_topic)
    data_plot.add_graph(['baro_alt_meter'], colors8[1:2], ['气压计高度'])
    ...
    for axis in ['x', 'y', 'z']:
        data_plot = DataPlot(data, plot_config, 'vehicle_local_position',
                             y_axis_label='[m]', title='飞机位置 '+axis.upper(),
                             plot_height='small', changed_params=changed_params,
                             x_range=x_range)
        data_plot.add_graph([axis], colors2[0:1], [axis.upper()+' 解算值'], mark_nan=True)
        data_plot.change_dataset('vehicle_local_position_setpoint')
        data_plot.add_graph([axis], colors2[1:2], [axis.upper()+' 设定值'],
                            mark_nan=True, use_step_lines=True)
        plot_flight_modes_background(data_plot, flight_mode_changes, vtol_states)
    ...
    try:
        ...
        if np.amax(cur_dataset.data['indicated_airspeed_m_s']) > 0.1:
            ...
            data_plot.add_graph(['indicated_airspeed_m_s'], colors2[0:1], ['指示空速'])
    ...
    data_plot = DataPlot(data, plot_config, 'actuator_controls_0',
                         y_start=0, title='旋翼控制信号', plot_height='small',
                         changed_params=changed_params, x_range=x_range)
    data_plot.add_graph(['control[0]', 'control[1]', 'control[2]', 'control[3]'],
                        colors8[0:4], ['滚转', '俯仰', '航向', '油门'], mark_nan=True)
    plot_flight_modes_background(data_plot, flight_mode_changes, vtol_states)
    if data_plot.finalize() is not None: plots.append(data_plot)
    ...
    data_plot = DataPlot(data, plot_config, 'actuator_outputs',
                         y_start=0, title='主通道输出', plot_height='small',
                         changed_params=changed_params, x_range=x_range)
    ...
    data_plot = DataPlot(data, plot_config, 'actuator_outputs',
                         y_start=0, title='副通道输出', plot_height='small',
    ...
    data_plot = DataPlot(data, plot_config, 'sensor_combined',
                         y_axis_label='[m/s^2]', title='加速度计原始值',
                         plot_height='small', changed_params=changed_params,
                         x_range=x_range)
    ...
    # raw angular speed
    data_plot = DataPlot(data, plot_config, 'sensor_combined',
                         y_axis_label='[deg/s]', title='陀螺仪原始值',
                         plot_height='small', changed_params=changed_params,
                         x_range=x_range)
    data_plot.add_graph([
    ...
    data_plot = DataPlot(data, plot_config, magnetometer_ga_topic,
                         y_axis_label='[gauss]', title='磁力计原始值',
                         plot_height='small', changed_params=changed_params,
                         x_range=x_range)
    ...
    data_plot = DataPlot(data, plot_config, 'distance_sensor',
                         y_start=0, y_axis_label='[m]', title='定高模块',
                         plot_height='small', changed_params=changed_params,
                         x_range=x_range)
    ...
    data_plot = DataPlot(data, plot_config, 'vehicle_gps_position',
                         title='GPS统计', y_range=Range1d(0, 40),
                         plot_height='small', changed_params=changed_params,
                         x_range=x_range)
    ...
    data_plot = DataPlot(data, plot_config, magnetometer_ga_topic,
                         y_start=0, title='油门与磁场噪音', plot_height='small',
                         changed_params=changed_params, x_range=x_range)
    data_plot.add_graph(
    ...
    try:
        ...
        input_data = [
            ...
            ('系统标志 (vel, pos, hgt)', estimator_status['health_flags']),
            ('超时标志 (vel, pos, hgt)', estimator_status['timeout_flags']),
            ('速度校验位', (estimator_status['innovation_check_flags'])&0x1),
            ('水平位置校验位', (estimator_status['innovation_check_flags']>>1)&1),
            ('垂直速度校验位', (estimator_status['innovation_check_flags']>>2)&1),
            ('磁力计校验位', (estimator_status['innovation_check_flags']>>3)&0x7),
